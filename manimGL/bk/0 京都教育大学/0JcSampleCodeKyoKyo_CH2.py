...
#  intro scene
class KyoEduIntroScene(JcSet):
    def construct(self):

        self.play(Write(self.ch0))

class Contents(JcSet):
    def construct(self):
        V=VGroup(self.ch1,self.ch2,self.ch3,self.ch4,self.ch5,self.ch6,self.ch7).arrange_submobjects(DOWN)
        # V=VGroup(self.ch1,self.ch2,self.ch3,self.ch4,self.ch5,self.ch6,self.ch6_1,self.ch7).arrange_submobjects(DOWN)

        self.play(Write(V))

class ContentsE(JcSet):
    def construct(self):
        V=VGroup(self.ch1,self.ch2,self.ch3,self.ch4,self.ch5,self.ch6,self.ch7).arrange_submobjects(DOWN)
        V_E=VGroup(self.ch1_E,self.ch2_E,self.ch3_E,self.ch4_E,self.ch5_E,self.ch6_E,self.ch7_E).arrange_submobjects(DOWN)
        # V=VGroup(self.ch1,self.ch2,self.ch3,self.ch4,self.ch5,self.ch6,self.ch6_1,self.ch7).arrange_submobjects(DOWN)
        ...
        self.play(Write(V_E))
        self.wait()
        self.play(FadeOut(V_E))
        self.wait()



        for i in range(7):
            self.play(Write(V[i],run_time=.5, rate_func = rush_into))
            self.wait(2)
            self.play(Transform(V[i], V_E[i].set_color(BLUE)))
            self.wait(1)


class ContentsE1(JcSet):
    CONFIG={
    "ch6_E":TextMobject("Chapter6 : Various test\\\\(Normality test, $\\chi^2$" ,"test etc..)")
    }
    def construct(self):

        aa=TextMobject("\\Ja{統計の用語は英語と対応づけて\\\\覚えると理解が深まります}").scale(2)


        self.play(GrowFromCenter(aa))
        self.wait()
        self.play(FadeOutAndShiftDown(aa))
        self.wait()
        ...
        V.scale(2/3).to_edge(LEFT).set_color_by_gradient(BLUE, YELLOW)
        V_E.scale(2/3).to_edge(RIGHT).set_color_by_gradient(BLUE, YELLOW)

        self.play(Write(V), Write(V_E))
...
class test(Scene):
    def construct(self):
        ...
        # self.add(t.scale(1/2))
        self.play(Write(t.scale(1/2)))

        self.add(t[0].to_corner(UL))
        self.add(t[1].to_corner(UL))
