...
class ss(GraphScene):
    def construct(self):
        ...
        self.play(ShowCreation(aa), rate_func=smooth, run_time=3)
        self.wait()

        bb = VGroup(*self.mobjects)
        bb.scale(.5)

        bb = self.get_graph(lambda x: x)

        self.add(bb)

        # self.play(bb.shift, DOWN)
        # self.wait()

        # self.play(aa,bb.shift))
        # self.wait()
...
class temp1(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(mob1), ShowCreation(mob2))
        self.wait()
        for i in range(3):
            self.play(mob1.shift, 3 * DOWN,  mob1.scale, 3,
                      rate_func=there_and_back, run_time=3)
        self.wait()

        self.play(mob1.rotate, PI / 3, about_point=np.array([2, 2, 0]))
        self.wait()
        #
        # self.add(name)
        # self.wait()
        # self.play(Uncreate(mob1))
        # self.wait()

        #
        # mob2.scale(.5).to_edge(BOTTOM)
        #
        # self.add(mob2)
        # self.wait()
        # self.wait()
...
class camera_ex(MovingCameraScene):
    def construct(self):
        screen_grid = ScreenGrid()
        self.add(screen_grid)

        # mob1=Dot(fill_opacity=.1)
        # rect.set_fill(YELLOW, 0.5)

        mob1 = Square().set_color(RED).set_fill(YELLOW, 0.1)
        mob2 = Square()

        # Arrange the objects
        VGroup(mob1, mob2).arrange_submobjects(RIGHT, buff=3)
        ...
        self.play(camera.move_to, mob1, camera.scale, .3)
        self.play(camera.scale, 3)

        self.wait()
        # camera.move_to(mob1)

        #
        # camera.set_width(mob2.get_width()*0.1)
        #
        #
        #
        # self.add(mob2)
        ...
        #
        #
        # self.play(camera.move_to, mob2, camera.scale, 3)
        # self.wait()
        #
        # self.play(Restore(camera))
        # self.wait()

        # Animation of the camera
        # self.play(
        #     # Set the size with the width of a object
        #     camera.set_width,mob1.get_width()*1.2,
        ...
        #
        # self.play(
        #     camera.set_height,square.get_width()*1.2,
        #     camera.move_to,square
        # )
        # self.wait()
        #
        # self.play(Restore(camera))
...
class aaa(Scene):
    def construct(self):
        # screen_grid = ScreenGrid()
        # self.add(screen_grid)
        aa=BarChart(values=[.1,.1], bar_names=["a", "b"])
        self.play(Write(aa))
        self.wait()
